...
const submit=async()=>{try{loading.value=true;error.value='';console.log({
  email: email.value,
  password: password.value
});await auth.login(email.value,password.value);router.push('/dashboard')}catch(e){error.value=e?.response?.data?.message||'Falha no login'}finally{loading.value=false}}
</script>
